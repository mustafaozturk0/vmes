...
export function useColorModeAndTheme(setMode: Function, mode: ThemeModes) {
  const colorMode = useMemo(
    () => ({
      toggleColorMode: () => {
        setMode((prevMode: ThemeModes) =>
  ...
  const theme = useMemo(
    () => createTheme(mode === ThemeModes.Light ? PureLightTheme : DarkTheme),
    [mode]
  );
